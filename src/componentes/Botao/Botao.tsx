...
const BotaoPrimarioEstilizado = styled.button<{ $width?: boolean }>`
...
`;
const BotaoSecundarioEstilizado = styled.button<{ $width?: boolean }>`
  width: ${({ $width }) => $width ? "100%" : "auto"};
...
export const Botao = ({ children, variante = 'primaria', tipo = "button", btnAtivo = false, isTotWidth }: PropsBotao) => {
  if (variante === 'primaria') {
    return <BotaoPrimarioEstilizado $width={isTotWidth} type={tipo} disabled={btnAtivo} >
      {children}
    </BotaoPrimarioEstilizado>
  }
  return <BotaoSecundarioEstilizado $width={isTotWidth}>
    {children}
